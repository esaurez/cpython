...
def _generate_setup_local_cmd() -> str:
    ...
    return (
        ...
        f"'# Group A (libz, libbz2, liblzma, libsqlite3, libssl, libcrypto)' "
        f"'# live in $(SYSROOT)/lib and are pulled into python.elf via' "
        f"'# --whole-archive (see Makefile.nanvix LIBS).' "
        f"'# Phase 3b (libffi): libffi.so ships separately under' "
        f"'# $(SYSROOT)/lib/libffi.so and _ctypes.cpython-312.so references' "
        f"'# it via DT_NEEDED; the loader resolves it at dlopen time and' "
        f"'# binds libffi UND symbols against python.elf .dynsym.' "
        f"'_bz2 _bz2module.c' "
