...
def package(
    ...
    *,
    ...
    # from the buildroot into the release ramfs at sysroot/lib/<name>.so
    # so the corresponding C extensions
    # (_lxml_etree.cpython-312.so, _lxml_elementpath.cpython-312.so,
    # _ctypes.cpython-312.so) can dlopen them at runtime.
    ...
    # at first `import lxml.etree` / `import ctypes`, so fail loudly
    # here instead of warning and shipping the broken artifact.
    ...
    if missing:
        raise FileNotFoundError(
            "Cannot stage release: required shared libraries missing "
